...
export default function AdminDoorsPage() {
  ...
  const filtered = sampleDoors.filter((d) => {
    const countMatch = count === '' || String(d.count).includes(count);
    const vtMatch = vehicleType === 'all' || d.vehicleTypes?.includes(Number(vehicleType));
    return countMatch && vtMatch;
  });
  ...
  return (
    <div className="container mx-auto px-4">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Doors</h1>
          <p className="mt-1 text-slate-600 dark:text-slate-400">Manage door counts.</p>
        </div>
        <Link
          to="/admin/masters/doors/add"
      ...
      {filtered.length ? (
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {filtered.map((d) => (
                <tr key={d.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/40">
                  <td className="p-4 font-medium">{d.count}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-right">
                    <Link
                      to={`/admin/masters/doors/edit/${d.id}`}
        ...
        <div className="rounded-lg bg-white p-6 text-center shadow-md dark:bg-slate-800">No Door records found.</div>
      )}
    </div>
  );
}
