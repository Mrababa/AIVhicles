...
export default function AdminFuelTypesPage() {
  ...
  const filtered = sampleFuelTypes.filter((f) => {
    const term = search.toLowerCase();
    const matchesSearch =
      f.nameEn.toLowerCase().includes(term) || f.nameAr.toLowerCase().includes(term);
    const matchesVehicleType =
      vehicleType === 'all' || f.vehicleTypes?.includes(Number(vehicleType));
  ...
  return (
    <div className="container mx-auto px-4">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Fuel Types</h1>
          <p className="mt-1 text-slate-600 dark:text-slate-400">Manage fuel type records.</p>
        </div>
        <Link
          to="/admin/masters/fuel-types/add"
      ...
      {filtered.length ? (
                ...
                <tr key={f.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/40">
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      {f.iconUrl && (
                        <img src={f.iconUrl} alt="" className="h-6 w-6" />
                      )}
                      <div>
                        <div className="font-medium text-slate-900 dark:text-slate-100">{f.nameEn}</div>
                        <div className="text-sm text-slate-500" dir="rtl">{f.nameAr}</div>
                      </div>
                    </div>
                  </td>
                  <td className="p-4">
                    <div className="flex flex-wrap gap-1">
                      {f.vehicleTypes?.map((id) => {
                  ...
                  <td className="p-4"><StatusBadge active={f.active} /></td>
                  <td className="p-4 text-right">
                    <Link
                      to={`/admin/masters/fuel-types/edit/${f.id}`}
        ...
        <div className="rounded-lg bg-white p-6 text-center shadow-md dark:bg-slate-800">No Fuel Types found.</div>
      )}
    </div>
  );
}
