...
export default function AdminTransmissionsPage() {
  ...
  const filtered = sampleTransmissions.filter((t) => {
    ...
    const matchesVehicleType =
      vehicleType === 'all' || t.vehicleTypes?.includes(Number(vehicleType));
    return matchesTerm && matchesVehicleType;
  });
  ...
  return (
    <div className="container mx-auto px-4">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Transmissions</h1>
          <p className="mt-1 text-slate-600 dark:text-slate-400">Manage transmission records.</p>
        </div>
        <Link
          to="/admin/masters/transmissions/add"
      ...
      {filtered.length ? (
          ...
          <table className="min-w-full">
            <thead className="bg-slate-50 dark:bg-slate-700/50">
              <tr>
                ...
                <th className="p-4 text-left text-sm font-semibold text-slate-500 dark:text-slate-400">Abbreviation</th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {filtered.map((t) => (
                <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/40">
                  ...
                  <td className="p-4">{t.abbreviation}</td>
                  <td className="p-4">
                    <div className="flex flex-wrap gap-1">
                      {t.vehicleTypes?.map((id) => {
                  ...
                  <td className="p-4 text-right">
                    <Link
                      to={`/admin/masters/transmissions/edit/${t.id}`}
        ...
        <div className="rounded-lg bg-white p-6 text-center shadow-md dark:bg-slate-800">No Transmissions found.</div>
      )}
    </div>
  );
}
