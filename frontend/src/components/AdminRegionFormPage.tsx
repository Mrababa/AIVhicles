...
export default function AdminRegionFormPage() {
  ...
  useEffect(() => {
    if (isEdit) {
      const existing = sampleRegions.find((r) => r.id === Number(regionId));
      if (existing) {
        setForm({ name: existing.name, countries: existing.countries || '', active: existing.active });
      }
    }
  }, [isEdit, regionId]);
  ...
  return (
    ...
    >
      ...
      <h1 className="mb-6 text-2xl font-bold">{isEdit ? 'Edit Region' : 'Add Region'}</h1>
      <div className="space-y-6">
        ...
        <div>
          <label htmlFor="countries" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
            List of Countries
          </label>
          <textarea
            id="countries"
            ...
            className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:border-slate-600 dark:bg-slate-700 sm:text-sm"
          />
          <p className="mt-1 text-xs text-slate-500">Enter a comma-separated list of countries, e.g., UAE, KSA, Bahrain</p>
        </div>
