...
export default function AdminDriveTrainFormPage() {
  const { driveTrainId } = useParams();
  const navigate = useNavigate();
  const isEdit = Boolean(driveTrainId);
  ...
  useEffect(() => {
    if (isEdit) {
      const existing = sampleDriveTrains.find((d) => d.id === Number(driveTrainId));
      if (existing) {
        setForm({
  ...
  const validate = () => {
    const exists = sampleDriveTrains.some(
      (d) => d.nameEn.toLowerCase() === form.nameEn.toLowerCase() && (!isEdit || d.id !== Number(driveTrainId))
    );
    if (exists) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) return;
    navigate('/admin/masters/drive-trains');
  };
  ...
  return (
    ...
    >
      ...
      <h1 className="mb-6 text-2xl font-bold">{isEdit ? 'Edit Drive Train' : 'Add Drive Train'}</h1>
      <div className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <button
            type="button"
            onClick={() => navigate('/admin/masters/drive-trains')}
