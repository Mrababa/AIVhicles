...
export default function AdminSeatFormPage() {
  ...
  useEffect(() => {
    if (isEdit) {
      const existing = sampleSeats.find((s) => s.id === Number(seatId));
      if (existing) {
        setForm({
  ...
  return (
    ...
    >
      ...
      <h1 className="mb-6 text-2xl font-bold">{isEdit ? 'Edit Seat Configuration' : 'Add Seat Configuration'}</h1>
      <div className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:border-slate-600 dark:bg-slate-700 sm:text-sm"
          />
          <p className="mt-1 text-xs text-slate-500">e.g., '2+2 Seating', 'Captain Chairs'</p>
        </div>
        <div>
        ...
        <div className="flex justify-end gap-3">
          <button
            type="button"
            onClick={() => navigate('/admin/masters/seats')}
