...
export default function AdminBodyTypeFormPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (isEdit) {
      const existing = sampleBodyTypes.find((b) => b.id === Number(id));
      if (existing) {
        setForm({
  ...
  const validate = () => {
    const exists = sampleBodyTypes.some(
      (b) => b.nameEn.toLowerCase() === form.nameEn.toLowerCase() && (!isEdit || b.id !== Number(id))
    );
    if (exists) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) return;
    navigate('/admin/masters/body-types');
  };

  const toggleVehicleType = (vtId: number) => {
    setForm((prev) => {
      const next = prev.vehicleTypes.includes(vtId)
        ? prev.vehicleTypes.filter((i) => i !== vtId)
        : [...prev.vehicleTypes, vtId];
  ...
  return (
    ...
    >
      ...
      <h1 className="mb-6 text-2xl font-bold">{isEdit ? 'Edit Body Type' : 'Add Body Type'}</h1>
      <div className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <button
            type="button"
            onClick={() => navigate('/admin/masters/body-types')}
